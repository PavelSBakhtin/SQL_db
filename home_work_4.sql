...
INSERT INTO media_types (name_type) VALUES ('Photo'), ('Music'), ('Video'), ('Post');
CREATE TABLE media (
	id SERIAL PRIMARY KEY,
...
/* Подсчитать общее количество лайков, которые получили пользователи младше 13 лет */
-- Вариант таблицы с данными:
SELECT user_id AS 'ID',
(SELECT CONCAT(firstname, ' ', lastname) FROM users WHERE id = likes.user_id) AS 'Пользователь',
(SELECT TIMESTAMPDIFF(YEAR, birthday, NOW()) FROM profiles WHERE user_id = likes.user_id) AS 'Возраст',
COUNT(*) AS 'Лайки' FROM likes WHERE
(SELECT TIMESTAMPDIFF(YEAR, birthday, NOW()) FROM profiles WHERE
user_id = likes.user_id AND (TIMESTAMPDIFF(YEAR, birthday, NOW()) < 13))
GROUP BY user_id ORDER BY user_id;
-- Вариант вывода только количества лайков по условию:
SELECT COUNT(*) AS 'Лайки' FROM likes WHERE (SELECT TIMESTAMPDIFF(YEAR, birthday, NOW()) FROM profiles
WHERE user_id = likes.user_id AND (TIMESTAMPDIFF(YEAR, birthday, NOW()) < 13));

/* Определить кто больше поставил лайков (всего): мужчины или женщины */
SELECT DISTINCT
(SELECT SUM(user_id) FROM likes WHERE id IN (SELECT user_id FROM profiles WHERE gender = 'f')) AS 'females',
(SELECT SUM(user_id) FROM likes WHERE id IN (SELECT user_id FROM profiles WHERE gender = 'm')) AS 'males';

/* Вывести всех пользователей, которые не отправляли сообщения */
SELECT CONCAT(firstname, ' ', lastname) AS 'do not send' FROM users
...
-- для задачи выбран пользователь 1, так как ему больше всех других прислали сообщения друзья:
SELECT from_user_id id, (SELECT CONCAT(firstname,' ', lastname) FROM users WHERE id = messages.from_user_id) AS 'user',
COUNT(*) AS send FROM messages WHERE to_user_id = 1 AND from_user_id IN (
SELECT initiator_user_id FROM friend_requests WHERE (target_user_id = 1) and status ='approved'
UNION
SELECT target_user_id FROM friend_requests WHERE (initiator_user_id = 1) and status ='approved')
GROUP BY from_user_id ORDER BY send DESC LIMIT 1;
